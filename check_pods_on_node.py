...
parser = argparse.ArgumentParser(prog='pods_on_node.py', usage='\n\t %(prog)s --url <openshift_master> --node <node_name> --token <bearer_token> --warn <int_of_pods> --crit <int_of_pods>', description='Nagios plugin to get quantity of pods in node', formatter_class=lambda prog: argparse.HelpFormatter(prog,max_help_position=30))
...
if pods < args.warn:
	print "OK - running " + str(pods) + " pod(s). | Pods=" + str(pods)
	sys.exit(0)
elif pods >= args.warn and pods < args.crit:
	print "WARNING - running " + str(pods) + " pod(s). | Pods=" + str(pods)
        sys.exit(1)
elif pods >= args.crit:
	print "CRITICAL - running " + str(pods) + " pod(s). | Pods=" + str(pods)
	sys.exit(2)
else:
	print "UNKNOWN - running " + str(pods) + " pod(s). | Pods=" + str(pods)
	sys.exit(3)
